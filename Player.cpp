...
bool Player::isValidMove(Move m) const
{
    ...
    auto isValidCastling = [this](const Piece* p, Coordinates to)
    {
        ...
        {
            bool isAccessed = enemy->isAccessedSquare({ Horizontal::E, _king->coord().y });
            if (to.x == Horizontal::G)
            {
                isAccessed &= enemy->isAccessedSquare({ Horizontal::F, _king->coord().y });
                return rightRook->isFirstMove() && rightRook->isPossibleMove({ Horizontal::F, to.y }) && !isAccessed;
            }
            if (to.x == Horizontal::C)
            {
                isAccessed &= enemy->isAccessedSquare({ Horizontal::D, _king->coord().y });
...
std::vector<Coordinates> Player::kingAccessedSquares() const
{
